...
export function setupAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    try {
      ...
      if (authType === 'ldap') {
        ...
        // JWT authentication
        const foundUser = await authService.storage.getUserByUsername(username);
        if (!foundUser || !foundUser.password) {
          return res.status(401).json({ message: "Invalid credentials" });
        }

        const isValid = await authService.comparePassword(password, foundUser.password);
        if (!isValid) {
          return res.status(401).json({ message: "Invalid credentials" });
        }
        user = foundUser;
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      const existingUser = await authService.storage.getUserByUsername(userData.username);
      if (existingUser) {
      ...
      const user = await authService.storage.createUser({
        id: `user-${Date.now()}`,
  ...
  app.get("/api/auth/config", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/auth/config", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update auth configuration" });
    }
  });
